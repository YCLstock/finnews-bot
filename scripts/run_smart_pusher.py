...
import logging
from core.utils import get_current_taiwan_time, format_taiwan_datetime, create_push_summary_message, setup_logger
from core.delivery_manager import get_delivery_manager
from core.topics_mapper import topics_mapper

# 設定 logger
logger = setup_logger(__name__, log_file="smart_pusher.log")
...
class SmartPusher:
    ...
    def get_eligible_articles_for_user(self, subscription: Dict[str, Any], hours_limit: int = 6) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[DB] 查詢到 {len(all_articles)} 篇最近 {hours_limit} 小時的文章")

            if not all_articles:
                print(f"[INFO] 沒有找到最近 {hours_limit} 小時的文章")
                return []
            ...
            print(f"[FILTER] 過濾後未推送文章: {len(unread_articles)} 篇")

            if not unread_articles:
                print(f"[INFO] 沒有新的未推送文章")
                return []
    ...
    def select_articles_by_preference(self, articles: List[Dict[str, Any]], keywords: List[str], max_count: int) -> List[Dict[str, Any]]:
        ...
        for art in scored_articles[:5]:
             print(f"   - [{art['relevance_score']:.2f}] {art['title'][:50]}...")
    ...
    def process_user_subscription(self, subscription: Dict[str, Any]) -> bool:
        ...
        print(f"\n--- [INFO] 處理用戶 {user_id[:8]}... 的推送 ({frequency_type}) ---")

        if not db_manager.should_push_now(subscription):
            print(f"[SKIP] 用戶 {user_id[:8]}... 不在推送時間窗口")
            return False
        ...
        if len(articles) < min_articles:
            print(f"[SKIP] 用戶 {user_id[:8]}... 沒有足夠的新文章 (需要至少 {min_articles} 篇)")
            print(f"[SKIP] 跳過本次推送，等待下次推送時間")
            return False

        delivery_platform = subscription.get('delivery_platform', 'discord')
        delivery_target = subscription['delivery_target']
        print(f"[PUSH] 開始推送 {len(articles)} 篇文章到 {delivery_platform}...")
        ...
        if success:
            ...
            for article in successful_articles:
                try:
                    ...
                    print(f"[WARN] 查找文章ID失敗: {e}")

            if article_ids:
                batch_success = db_manager.log_push_history(user_id, article_ids)
                if batch_success:
                    print(f"[DB] 已記錄推送歷史: {len(article_ids)} 篇文章")
                ...
                print(f"[SUCCESS] 用戶 {user_id[:8]}... 推送完成: {len(successful_articles)} 篇成功")
                return True

        print(f"[FAIL] 用戶 {user_id[:8]}... 推送失敗")
        return False

    def run_smart_push(self) -> bool:
        """執行智能推送"""
        print("=" * 60)
        print("FinNews-Bot 智能推送器開始執行 (v2.0 - 混合評分引擎)")
        ...
        try:
            ...
            if not eligible_subscriptions:
                ...
                print("[INFO] 提示: 系統會在 08:00, 13:00, 20:00 (±30分鐘) 進行推送檢查")
                return True
            ...
            if overall_success:
                ...
                print("[INFO] 可能原因: 沒有足夠的新文章，或用戶已在此時間窗口收到推送")
